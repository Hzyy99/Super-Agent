...
from harness.config.runtime_paths import resolve_path
from harness.skills.storage.skill_storage import SKILL_MD_FILE, SkillStorage
...
class LocalSkillStorage(SkillStorage):
    ...
    def _iter_skill_files(self) -> Iterable[tuple[SkillCategory, Path, Path]]:
        ...
        for current_root, dir_names, file_names in os.walk(self._host_root, followlinks=True):
            dir_names[:] = sorted(name for name in dir_names if not name.startswith(".") and name != SkillCategory.CUSTOM.value)
            if SKILL_MD_FILE not in file_names:
                continue
            yield SkillCategory.PUBLIC, self._host_root, Path(current_root) / SKILL_MD_FILE

        custom_path = self._host_root / SkillCategory.CUSTOM.value
        if custom_path.exists() and custom_path.is_dir():
            for current_root, dir_names, file_names in os.walk(custom_path, followlinks=True):
                dir_names[:] = sorted(name for name in dir_names if not name.startswith("."))
                if SKILL_MD_FILE not in file_names:
                    continue
                yield SkillCategory.CUSTOM, custom_path, Path(current_root) / SKILL_MD_FILE
