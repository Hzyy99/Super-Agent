...
skill_manage_module = importlib.import_module("harness.tools.skill_manage_tool")
...
def test_skill_manage_create_and_patch(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("harness.skills.security_scanner.get_app_config", lambda: config)
    refresh_calls = []
...
def test_skill_manage_rejects_public_skill_patch(monkeypatch, tmp_path):
    skills_root = tmp_path / "skills"
    public_dir = skills_root / "deep-research"
