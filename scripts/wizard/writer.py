...
def _default_tools() -> list[dict[str, Any]]:
    return [
        {"name": "image_search", "use": "harness.community.image_search.tools:image_search_tool", "group": "web", "max_results": 5},
        {"name": "ls", "use": "harness.sandbox.tools:ls_tool", "group": "file:read"},
        {"name": "read_file", "use": "harness.sandbox.tools:read_file_tool", "group": "file:read"},
        {"name": "glob", "use": "harness.sandbox.tools:glob_tool", "group": "file:read"},
        {"name": "grep", "use": "harness.sandbox.tools:grep_tool", "group": "file:read"},
        {"name": "write_file", "use": "harness.sandbox.tools:write_file_tool", "group": "file:write"},
        {"name": "str_replace", "use": "harness.sandbox.tools:str_replace_tool", "group": "file:write"},
        {"name": "bash", "use": "harness.sandbox.tools:bash_tool", "group": "bash"},
    ]
...
def _build_tools(
    *,
    ...
    if include_bash_tool:
        tools.append({"name": "bash", "use": "harness.sandbox.tools:bash_tool", "group": "bash"})
...
def build_minimal_config(
    *,
    ...
    sandbox_use: str = "harness.sandbox.local:LocalSandboxProvider",
    ...
    if sandbox_use == "harness.sandbox.local:LocalSandboxProvider":
